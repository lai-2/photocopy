...
title inMate - Cai dat tu dong

:: ============================================================================
:: install-inmate.cmd - Bo cai dat tu dong cho ung dung inmate
:: ============================================================================
...
::      README-inmate.md "Cai dat tren Windows 7" da ghi. Khong the tu dong
...
::      xong thi chay lai chinh file install-inmate.cmd nay de tiep tuc (cac buoc da
...
::      de cac icon dang emoji trong giao dien (tu ban inmate moi, xem
...
::      thanh cong.
::   5. Tai ban inmate moi nhat tu GitHub Releases - dung dung nguon va cach
...
::      photocopy, asset "inmate-<version>.zip" trong release "latest") - va
::      giai nen vao C:\Program Files\inmate.
::   6. Tao shortcut "inmate" ngoai Desktop.
::
...
:: trong cung thu muc voi file install-inmate.cmd nay, dong thoi van hien tren
:: man hinh nhu binh thuong - de tien kiem tra sau nay, dac biet huu ich khi
:: debug tren Windows 7.
:: ============================================================================

set "INSTALL_DIR=C:\Program Files\inmate"
set "TEMP_DIR=%TEMP%\InMateInstall"
...
echo     Sau khi cai xong SP1 va KHOI DONG LAI may, chay lai file install-inmate.cmd
...
echo     trinh duyet. Cai xong thi chay lai install-inmate.cmd nay de tiep tuc.
echo.
...
if errorlevel 1 (
    echo [LOI] Khong tai duoc bo cai .NET Framework 4.8. Kiem tra ket noi mang roi chay lai install-inmate.cmd.
    pause
    exit /b 1
)
...
:: ---- Buoc 4: tai va cai dat ung dung inmate ----------------------------------
:download_app
echo.
echo [*] Dang tim va tai ban inMate moi nhat tu GitHub...
...
if not errorlevel 1 (
    ...
    echo       Kiem tra ket noi mang roi chay lai install-inmate.cmd.
    pause
    exit /b 1
)
if not defined APP_EXTRACT (
    echo [LOI] Khong tai/giai nen duoc ban inmate moi nhat.
    pause
    exit /b 1
)
...
echo.
echo [*] Dang tao shortcut inMate ngoai Desktop...
powershell -NoProfile -Command "$ws = New-Object -ComObject WScript.Shell; $desktop = [Environment]::GetFolderPath('Desktop'); $lnk = $ws.CreateShortcut([System.IO.Path]::Combine($desktop, 'inmate.lnk')); $lnk.TargetPath = '%INSTALL_DIR%\inmate.exe'; $lnk.WorkingDirectory = '%INSTALL_DIR%'; $lnk.IconLocation = '%INSTALL_DIR%\inmate.exe'; $lnk.Save()"
if errorlevel 1 (
    echo [!] Khong tao duoc shortcut - co the tu tao thu cong tu "%INSTALL_DIR%\inmate.exe".
) else (
    echo [OK] Da tao shortcut inMate ngoai Desktop.
)
